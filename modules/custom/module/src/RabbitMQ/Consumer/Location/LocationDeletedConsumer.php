...
/**
 * Verwerkt inkomende <LocationDeleted>-berichten.
 *
 * Queue      : planning.location.deleted
 * Exchange   : frontend.topic
 * Routing key: frontend.location.deleted
 */
class LocationDeletedConsumer {
  ...
  public function listen(string $queueName = 'planning.location.deleted'): void {
    echo "LocationDeletedConsumer luistert op '{$queueName}'...\n";
    \ControlRoomLogger::info('frontend-location-deleted', "LocationDeletedConsumer luistert op '{$queueName}'...");
    ...
    $this->channel->queue_bind($queueName, 'frontend.topic', 'frontend.location.deleted');
    ...
    $this->channel->basic_consume(
      ...
      function (AMQPMessage $msg) {
        try {
          ...
          echo "Fout bij verwijderen locatie: " . $e->getMessage() . "\n";
          \ControlRoomLogger::error('frontend-location-deleted', 'Fout: ' . $e->getMessage());
        }
      }
    );
  ...
  private function handleMessage(AMQPMessage $msg): void {
    ...
    try {
      $this->validator->validate($xml, 'location_deleted');
    }
    catch (\RuntimeException $e) {
      \Drupal::logger('rabbitmq')->error(
        'LocationDeleted XSD-validatie mislukt: @msg',
        ['@msg' => $e->getMessage()]
      );
      throw $e;
    }
    ...
    echo sprintf("[%s] Locatie verwijderd: ID %s\n",
      date('H:i:s'), $data['locationId']);
    
    \ControlRoomLogger::info('frontend-location-deleted', sprintf(
      'Locatie verwijderd: ID %s', $data['locationId']
    ));
  }
  ...
  private function upsertDrupalLocation(array $data): void {
    ...
    echo "Locatie Verwijdering Verwerken, ID: " . $data['locationId'] . "\n";
  }
